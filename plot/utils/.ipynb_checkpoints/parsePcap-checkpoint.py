...
class TsnParsePcap():
    ...
    def __init__(self, path, mapping, output_path, cycle_time=None, omnet_ts=False):
        self.path = path
        ...
        self.omnet_ts=omnet_ts

        try:
            if self.omnet_ts:
#                print("tshark -F pcap -r "+self.path+" -w "+self.path+"_omnet")
                os.system("tshark -F pcap -r "+self.path+" -w "+self.path+"_omnet") 
                self.path=self.path+"_omnet"
            f = open(self.path, "rb")
            self.pcap = Reader(f)
    ...
    def pcap_to_dfs(self):
        ...
        for ts, buf in self.pcap:
            ...
            if not self.omnet_ts:
                src = self.mapping[int(hexstream[12:24],16)]
            else:
                src = self.mapping[int(hexstream[20:24],16)]
            ...
            ts_recv = ts
            if self.omnet_ts:
                  ts_send= Decimal(int(hexstream[96:112], 16)/1000000000)
                  ts_recv = Decimal(int(hexstream[112:128], 16)/1000000000)
        ...
        if(self.omnet_ts):
            start = Decimal(ceil(max(starts)))
            end = Decimal(floor(min(ends)))
