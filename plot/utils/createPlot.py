...
class TsnCreatePlot():
    ...
    def generate_plots(self, plot_type, output="mpl", title=True,
        ...
        if type(self.dfs) == list and plot_type == "boxplot":
            ...
            if plot_type == "latency_cdf":
                ...
                self.cdf(measurement="Inter Frame Gap Sender (ns)",
                         complementary=False,
                         losses=lossesInMeasurement,
                         output=output,
                         plot_type=plot_type,
    ...
    def ifg_over_time(self, df, stream, losses, output,
        ...
        x = df['Timestamp Sender (Unix epoch, s)'].loc[
            df['Frame No.'] > offset] - floor(
            df['Timestamp Sender (Unix epoch, s)'].iloc[0])
        y = df['Inter Frame Gap Sender (ns)'].loc[df['Frame No.'] > offset]

        if output == "mpl":
            plt.plot(x, y)
            plt.axhline(y=df['Inter Frame Gap Sender (ns)'].loc[
    ...
    def cdf(self, measurement, complementary, losses,
        ...
        if output == "mpl":
            ...
            if title:
                ...
                plt.title("CCDF", **self.title_params_left) \
                    if complementary else plt.title("CDF",
